...
import { loginWithIdentity, logoutUser, registerCyberBullAccount, sendResetLinkForIdentity } from '@/services/authService'
...
type AuthContextValue = {
  ...
  resetPassword: (domain: string, username: string) => Promise<void>
  logout: () => Promise<void>
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (domain: string, username: string, password: string) => {
    setError(null)
    await loginWithIdentity(domain, username, password)
  }
  ...
  const resetPassword = async (domain: string, username: string) => {
    setError(null)
    await sendResetLinkForIdentity(domain, username)
  }
